...
// 远程控制权限管理
const remoteControlPermissions = new Map(); // user -> isAllowed
// 配置 nut-js (加快响应速度)
...
// ======================== 远程控制功能 ========================
// 请求远程控制权限
ipcMain.handle('request-remote-control', async (_event, data) => {
...
// 授予/撤销远程控制权限
ipcMain.handle('set-remote-control-permission', async (_event, data) => {
    remoteControlPermissions.set(data.user, data.allowed);
    console.log(`[远程控制] ${data.user} 权限 ${data.allowed ? '已授予' : '已撤销'}`);
    return { success: true };
});
// 执行远程鼠标移动
ipcMain.handle('remote-mouse-move', async (_event, data) => {
...
// 执行远程鼠标点击
ipcMain.handle('remote-mouse-click', async (_event, data) => {
    if (!remoteControlPermissions.get(data.user)) {
        return { success: false, error: 'Permission denied' };
    }
    try {
